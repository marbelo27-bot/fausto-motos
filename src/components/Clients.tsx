              ...
              {filtered.length === 0 ? (
                ...
                filtered.map(client => {
                  ...
                  return (
                    <tr key={client.id}>
                      <td>
                        <div style={{ fontWeight: 600, color: "#fff" }}>{client.fullName}</div>
                      </td>
                      <td>{client.phone}</td>
                      ...
                      <td style={{ fontWeight: 600, color: "#11A900" }}>
        ...
        <div className="modal-overlay" onClick={() => setViewingClient(null)}>
          <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button onClick={() => setViewingClient(null)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#888" }}>✕</button>
            </div>

            <div className="grid-2" style={{ marginBottom: 20 }}>
              <div>
                <div className="section-title">Información Personal</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8, color: "#ccc" }}>
                  <div><strong style={{ color: "#CAF404" }}>Teléfono:</strong> {viewingClient.phone}</div>
                  <div><strong style={{ color: "#CAF404" }}>Dirección:</strong> {viewingClient.address || "—"}</div>
                  <div><strong style={{ color: "#CAF404" }}>Fecha de ingreso:</strong> {new Date(viewingClient.createdAt).toLocaleDateString("es-AR")}</div>
                  {viewingClient.notes && <div><strong style={{ color: "#CAF404" }}>Notas:</strong> {viewingClient.notes}</div>}
                </div>
              </div>
              <div>
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, color: "#ccc" }}>
                      <div>🏍️ <strong style={{ color: "#fff" }}>{stats.motos}</strong> moto(s) registrada(s)</div>
                      <div>🔧 <strong style={{ color: "#fff" }}>{stats.orders}</strong> orden(es) de servicio</div>
                      <div>💰 Total pagado: <strong style={{ color: "#11A900" }}>${stats.totalPaid.toLocaleString("es-AR")}</strong></div>
                    </div>
                  );
                })()}
              </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {motorcycles.filter(m => m.clientId === viewingClient.id).length === 0 ? (
                <p style={{ color: "#555", fontSize: 13 }}>Sin motos registradas</p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {motorcycles.filter(m => m.clientId === viewingClient.id).map(m => (
                    <div key={m.id} style={{ padding: "8px 12px", background: "#111", borderRadius: 8, border: "1px solid #2a2a2a", fontSize: 13, color: "#ccc" }}>
                      🏍️ <strong style={{ color: "#fff" }}>{m.brand} {m.model}</strong> — {m.year} — Dom: <strong style={{ color: "#CAF404" }}>{m.plate}</strong>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {serviceOrders.filter(o => o.clientId === viewingClient.id).length === 0 ? (
                <p style={{ color: "#555", fontSize: 13 }}>Sin órdenes registradas</p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {serviceOrders.filter(o => o.clientId === viewingClient.id).map(o => (
                    <div key={o.id} style={{ padding: "8px 12px", background: "#111", borderRadius: 8, border: "1px solid #2a2a2a", fontSize: 13, display: "flex", justifyContent: "space-between", color: "#ccc" }}>
                      <span>🔧 {o.performedService} — {new Date(o.date + "T00:00:00").toLocaleDateString("es-AR")}</span>
                      <span style={{ fontWeight: 600, color: "#CAF404" }}>${o.totalCost.toLocaleString("es-AR")}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              {payments.filter(p => p.clientId === viewingClient.id).length === 0 ? (
                <p style={{ color: "#555", fontSize: 13 }}>Sin pagos registrados</p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {payments.filter(p => p.clientId === viewingClient.id).map(p => (
                    <div key={p.id} style={{ padding: "8px 12px", background: "#111", borderRadius: 8, border: "1px solid #2a2a2a", fontSize: 13, display: "flex", justifyContent: "space-between", color: "#ccc" }}>
                      <span>💰 {p.type} ({p.method}) — {new Date(p.date + "T00:00:00").toLocaleDateString("es-AR")}</span>
                      <span style={{ fontWeight: 600, color: "#11A900" }}>${p.amount.toLocaleString("es-AR")}</span>
                    </div>
                  ))}
                </div>
              )}
